...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    let hasEmpty = false || JSON.parse(localStorage.getItem("hasEmpty"));
    ...
    requiredFields.forEach(name => {
      ...
      if (field.value.trim() === "") {  
        ...
        localStorage.setItem('hasEmpty', JSON.stringify(true));
      } else {
        field.classList.remove("is-invalid");
        field.classList.add("is-valid");
        localStorage.setItem('hasEmpty', JSON.stringify(false));
      }
    });
